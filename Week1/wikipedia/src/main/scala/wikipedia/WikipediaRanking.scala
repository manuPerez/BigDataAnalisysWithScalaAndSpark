...
object WikipediaRanking {
  ...
  def occurrencesOfLang(lang: String, rdd: RDD[WikipediaArticle]): Int = rdd.isEmpty() match {
    case true => 0
    case _ => {
      rdd.filter(wiki => wiki.text.split(" ").contains(lang))
        .aggregate(0)((acc, y) => acc + 1, (acc1, acc2) => acc1 + acc2)
    }
  }
  ...
  def makeIndex(langs: List[String], rdd: RDD[WikipediaArticle]): RDD[(String, Iterable[WikipediaArticle])] =
      rdd.flatMap(wiki => langs.filter(lang => wiki.text.split(" ").contains(lang)).map(word => (word, wiki))).groupByKey()
  ...
  def rankLangsUsingIndex(index: RDD[(String, Iterable[WikipediaArticle])]): List[(String, Int)] =
      index.keyBy(_._1).mapValues(_._2.size).sortBy(_._2, false).collect().toList
  ...
  def rankLangsReduceByKey(langs: List[String], rdd: RDD[WikipediaArticle]): List[(String, Int)] =
    rdd.flatMap(wiki => langs.filter(lang => wiki.text.split(" ").contains(lang))).map(word => (word, 1))
